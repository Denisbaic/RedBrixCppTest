...
void WeaponSystem::execute(entt::registry & world,float dt)
{
	...
	for (auto [entity,transform_info, has_weapon_info,team_info] : units.each())
	{
		if(weapons_ready_to_shoot.contains(has_weapon_info.weapon))
		{
			...
			{
				float current_distance = Vector3Distance(transform_info.transform.translation, transform_info2.transform.translation);
			...
			{
				auto shoot_request = world.create();
